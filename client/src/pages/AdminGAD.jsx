...
function BookingTooltip({ active, payload, label }) {
  ...
  return (
    <div className="max-w-xs rounded-xl border border-purple-100 bg-white p-4 shadow-lg">
      <p className="mb-2 font-semibold text-purple-950">
        {label}
      </p>
...
async function getData(path, signal) {
  ...
  if (!response.ok || !Array.isArray(data)) {
    throw new Error(
      data?.error || "Unexpected GAD response. Please try again."
    )
  }
...
function AdminGAD() {
    ...
    async function load() {
      try {
        ...
        if (!controller.signal.aborted) {
          setError(
            requestError.message || "Unable to load GAD analytics."
          )
        }
      } finally {
        if (!controller.signal.aborted) {
  ...
  const totals = useMemo(() => {
    ...
    summary.forEach((item) => {
      ...
      result.set(
        gender,
        result.get(gender) + (Number(item.total) || 0)
      )
    })
  ...
  const customerTotal = [...totals.values()].reduce(
    (sum, count) => sum + count,
    0
  )
  ...
  const bookingTotal = services.reduce(
  ...
  const leadingServices = services.length
    ? services.filter(
        (item) => item.total === services[0].total
      )
    : []
  ...
  const displayedGroups = GROUPS.filter(
    (group) =>
      totals.get(group.name) > 0 ||
      preferences.some((item) => item.gender === group.name)
  )
        ...
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-purple-100">
              Explore recorded customer profiles and service
              bookings to support inclusive salon planning.
            </p>
      ...
      >
          ...
          <table className="w-full min-w-[620px] text-left text-sm">
            <thead className="bg-purple-50 text-purple-900">
              <tr>
                ...
                <th className="rounded-tr-xl px-4 py-3 text-right">
                  Bookings
                </th>
              </tr>
            </thead>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 text-sm">
          <p className="text-gray-500">
            {number(filtered.length)} groups · Page{" "}
        ...
        <div className="grid gap-4 md:grid-cols-2">
            ...
            {leadingServices.length ? (
              <>
                ...
                <p className="mt-2 text-sm leading-relaxed text-gray-600">
                  {number(leadingServices[0].total)} recorded
                  booking(s)
                  {leadingServices.length > 1 ? " each" : ""}.
                  This describes booking activity, not every
          ...
          <div className="rounded-xl bg-slate-50 p-5">
            ...
            <h4 className="mt-2 font-bold text-slate-900">
              {number(missingBookings)} booking(s) without
              recorded gender
            </h4>
            <p className="mt-2 text-sm leading-relaxed text-gray-600">
              This includes unmatched customer profiles and
              blank gender values. “Prefer not to say” is
              counted separately as an explicit recorded choice.
            </p>
          </div>
        </div>

        <p className="mt-5 text-xs leading-relaxed text-gray-500">
          Bookings use the latest customer profile matched by
          contact number. Shared numbers can associate bookings
          with the same profile. Customer totals count profile
          records and may include duplicates. These figures do
          not establish individual preferences or measure fairness
          of access to salon services.
        </p>
      </Panel>
    </div>
  )
