  ...
  const loadReport = useCallback(async (selectedPeriod, signal) => {
    ...
    try {
      const response = await apiFetch(
        `${API_URL}/reports?period=${selectedPeriod}`,
        { signal }
      )
    ...
    <div className="min-w-0 space-y-6">
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <article className={`${panel} p-5 md:p-6`}>
          ...
          <div className="mt-5 h-72">
            {!hasSales ? (
              <ChartEmpty>
                No paid sales recorded in the last seven days.
              </ChartEmpty>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={report.salesTrend}>
                  ...
                  <Tooltip
                    formatter={(value) => [money(value), "Paid sales"]}
                  />
      ...
      <section className={`${panel} p-5 md:p-6`}>
        ...
        <div className="mt-5 h-72">
          {!report.mostRequestedServices.length ? (
            <ChartEmpty>
              No service requests recorded for this period.
            </ChartEmpty>
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
      ...
      <section className={`${panel} overflow-hidden`}>
        <div className="p-5 md:p-6">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Sales count paid transactions; appointment count comes from
            booking records.
          </p>
        </div>
      ...
      <p className="text-xs text-slate-500">
        Report dates use Philippine time. All figures are descriptive records
        of completed system activity; no forecasting is used.
      </p>
    </div>
  )
}
