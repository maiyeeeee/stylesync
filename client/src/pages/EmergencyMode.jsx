...
function EmergencyMode() {
  ...
  useEffect(() => {
    ...
    try {
      const cached = JSON.parse(
        localStorage.getItem(REFERENCE_KEY) || "null"
      )
    ...
    Promise.all([read("/services"), read("/inventory")])
      ...
      .catch((err) => {
        if (!controller.signal.aborted) {
          setReferenceError(
            err.message || "Unable to refresh reference data."
          )
        }
      })
  ...
  async function saveRecord(event) {
    ...
    if (recordType === "appointment") {
      const selected = services.find(
        (item) =>
          Number(item.service_id) === Number(fields.get("service_id"))
      )
      ...
      payload = {
        ...
        sale_type: saleType,
        service:
          saleType === "Product" ? selected.name : selected.service,
  ...
  async function syncRecords() {
    ...
    try {
      ...
      const review = current.filter(
        (item) => item.sync_status === "Needs Review"
      ).length
      ...
      setError(
        err.message ||
          "Sync could not finish. Check the queue before retrying."
      )
  ...
  async function clearSynced() {
    ...
    if (
      !window.confirm(
        "Clear synced copies from this device? Unsynced records will be kept."
      )
    ) {
      return
    }
  ...
  const filteredRecords = records.filter(
    (item) =>
      statusFilter === "All" || item.sync_status === statusFilter
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-relaxed text-amber-900">
        Offline appointments are provisional. Staff availability and
        conflicts are checked during synchronization. Invalid slots
        need manual review. A network connection does not guarantee
          ...
          <fieldset disabled={queueBlocked} className="grid min-w-0 gap-4 md:grid-cols-2">
            {recordType === "appointment" && (
              <>
                ...
                <Field label="Contact number">
                  <input name="contact_number" type="tel" required placeholder="09XXXXXXXXX" className={inputClass} />
                </Field>
                <Field label="Email (optional)">
                  <input name="email" type="email" className={inputClass} />
                </Field>
                <Field label="Cached service">
                  <select name="service_id" required defaultValue="" className={inputClass}>
                    <option value="" disabled>Select service</option>
                    {availableServices.map((item) => (
                      <option key={item.service_id} value={item.service_id}>
                        {item.service} ({item.duration_minutes
                ...
                <Field label="Start time">
                  <input name="appointment_time" type="time" min="09:00" max="19:00" required className={inputClass} />
                </Field>
            ...
            {recordType === "inventory" && (
              <>
                ...
                <Field label="Quantity adjustment">
                  <input name="quantity_change" type="number" step="1" required placeholder="For example, -2 or 5" className={inputClass} />
                </Field>
                <div className="md:col-span-2">
                  <Field label="Reason">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 pt-4 md:col-span-2">
              <p className="max-w-lg text-xs leading-relaxed text-gray-500">
                Switching record types clears unsaved form entries.
                Saved records remain in this device’s queue.
              </p>
              <button
                type="submit"
      ...
      <section className="min-w-0 overflow-hidden rounded-2xl border border-purple-100 bg-white shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4 p-5">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              disabled={queueBlocked || !online || !records.some((item) => item.sync_status !== "Synced")}
              onClick={syncRecords}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 pb-4">
          <p className="text-xs text-gray-500">
            {loadingRecords ? "Loading queue…" : `${filteredRecords.length} records shown`}
          </p>
          <select
          ...
          >
            ...
            {["Pending", "Syncing", "Needs Review", "Failed", "Synced"].map((status) => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[850px] text-left text-sm">
            <thead className="border-y border-purple-100 bg-purple-50 text-purple-900">
              <tr>
                {["Local record", "Type", "Recorded", "Status", "Result"].map((label) => (
                  <th key={label} scope="col" className="px-5 py-3 text-xs font-semibold">
                    {label}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              ...
              {filteredRecords.map((record) => (
                <tr key={record.offline_id} className="border-b border-gray-100 hover:bg-purple-50">
                  ...
                  <td className="px-5 py-4 align-middle">
                    <span className={`inline-flex whitespace-nowrap rounded-full px-3 py-1 text-xs font-semibold ${statusClass(record.sync_status)}`}>
                      {record.sync_status}
                    </span>
                  </td>
        ...
        <p className="px-5 py-4 text-xs leading-relaxed text-gray-500">
          Keep unsynced records on this device. Do not clear browser
          site data while records are pending. “Clear synced copies”
          removes only local copies already synchronized.
        </p>
      </section>
    </div>
  )
    ...
    <>
      ...
      <Field label="Cached item / service">
        <select
          ...
          className={inputClass}
        >
          <option value="" disabled>Select item or service</option>
          {items.map((item) => {
