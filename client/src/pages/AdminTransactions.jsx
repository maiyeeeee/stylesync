...
const input =
  "w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100"
const button =
  ...
  return (
    ...
    >
      ...
      {open && !disabled && (
        ...
        >
          {matches.length === 0 ? (
            <li className="p-3 text-sm text-slate-500">No matches found.</li>
          ) : (
            matches.map((option, index) => (
              <li
                ...
                className={`cursor-pointer rounded-lg p-3 text-sm ${
                  index === active ? "bg-purple-50 text-purple-900" : "text-slate-700"
                }`}
              >
                {option.label}
              </li>
...
function AdminTransactions() {
  ...
  const appointment = appointments.find(
    (row) => String(row.id) === form.appointmentId
  )
  ...
  const deposit = booked && appointment?.payment_status === "Verified"
    ? Number(appointment.verified_deposit || 0)
    : 0

  const balanceCents = toCents(form.amount) - toCents(deposit)
  const unresolved = booked && appointment && (
  ...
  const itemOptions = items
    .filter((row) =>
      productSale ? Number(row.stock) > 0 : row.status === "Available"
    )
    .map((row) => ({
  ...
  function selectItem(id) {
    const row = items.find(
      (item) => String(productSale ? item.id : item.service_id) === id
    )
  ...
  async function save(event) {
    ...
    const payload = {
      ...
      service: booked
        ? appointment.service
        : productSale ? selectedItem.name : selectedItem.service,
    ...
    try {
      ...
      const saved = rows?.find(
        (row) => String(row.transaction_id) === String(result.id)
      )
  ...
  const todayRows = transactions.filter(
    (row) => dayKey(row.transaction_date) === dayKey(clock)
  )
  ...
  const filtered = transactions.filter((row) =>
    [row.customer, row.service, row.transaction_id, row.appointment_id]
      .some((value) =>
  ...
  return (
    <div className="min-w-0 space-y-6">
      ...
      {message && (
        <p role="status" className="rounded-xl bg-green-50 p-4 text-green-800">{message}</p>
      )}
      ...
      <div className="grid items-start gap-6 xl:grid-cols-2">
          ...
          <form onSubmit={save}>
            <fieldset disabled={disabled} className="space-y-4">
              ...
              {booked ? (
                <>
                  ...
                  <p className="text-xs text-slate-500">
                    Search by name, contact, booking number, or service.
                    Only approved/completed bookings without a transaction appear.
                  </p>
                  ...
                  {appointment && (
                    <div className="rounded-xl bg-purple-50 p-4 text-sm text-purple-900">
                      ...
                      <p className="mt-2">
                        Deposit status: {appointment.payment_status || "No deposit recorded"}
                      </p>
                    </div>
                  )}
                </>
                ...
                <>
                  ...
                  <SearchSelect
                    key={form.sale_type}
                    label={productSale ? "Find and select a product" : "Find and select a service"}
                    options={itemOptions}
                      ...
                      <input
                        ...
                        onChange={(event) => {
                          ...
                          setForm((previous) => ({
                            ...
                            quantity,
                            amount: selectedItem
                              ? (Number(selectedItem.price) * Number(quantity)).toFixed(2)
                              : "",
                          }))
                        }}
                      />
              ...
              <div className="space-y-3 rounded-xl bg-slate-50 p-4 text-sm">
                ...
                <div className="flex justify-between gap-3 border-t border-slate-200 pt-3 text-lg font-bold text-purple-900">
              ...
              {balanceCents < 0 && (
                <p className="text-sm text-red-700">
                  The deposit exceeds the final bill. Resolve the excess before saving.
                </p>
              )}
        ...
        <section className={panel}>
          ...
          {!receipt ? (
            <p className="mt-5 rounded-xl border border-dashed border-purple-200 p-10 text-center text-sm text-slate-500">
            ...
            <div className="mt-5 space-y-4 text-sm">
              ...
              <p className="text-slate-500">
                Receipt #{receipt.transaction_id} · {dateLabel(receipt.transaction_date)}
              </p>

              {[
                ["Customer", receipt.customer],
                ["Booking", receipt.appointment_id ? `#${receipt.appointment_id}` : "Walk-in / Product sale"],
                ...
                ["Balance payment", Number(receipt.balance_collected) > 0 ? receipt.payment : "No additional payment"],
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-left text-sm">
            <thead className="bg-slate-50 text-slate-500">
              <tr>
                {["Customer / Date", "Service / Item", "Sale total", "Deposit", "Balance collected", "Status", "Receipt"].map((title) => (
                  <th key={title} className="px-3 py-4">{title}</th>
                ))}
              </tr>
            </thead>
              ...
              {filtered.map((row) => (
                <tr key={row.transaction_id} className="hover:bg-purple-50/40">
                  <td className="px-3 py-4">
                    ...
                    <p className="mt-1 text-xs text-slate-500">{dateLabel(row.transaction_date)}</p>
                    {row.appointment_id && (
                      <p className="mt-1 text-xs text-purple-600">Booking #{row.appointment_id}</p>
                    )}
                  </td>
                  <td className="px-3 py-4">{row.service}</td>
                  <td className="whitespace-nowrap px-3 py-4">{money(row.amount)}</td>
                  <td className="whitespace-nowrap px-3 py-4 text-emerald-700">{money(row.deposit_applied)}</td>
                  <td className="whitespace-nowrap px-3 py-4">{money(row.balance_collected)}</td>
