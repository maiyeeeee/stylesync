...
function AdminRecommendations() {
    ...
    useEffect(() => {
        ...
        async function loadRecommendations() {
            try {
                const response = await apiFetch(`${API_URL}/recommendations`, {
                ...
                if (!response.ok) {
                    throw new Error(result?.error || "Unable to load recommendations.")
                }
                ...
                if (!controller.signal.aborted) {
                    setError(
                        requestError.message || "Unable to load recommendations. Please try again.",
                    )
                }
            } finally {
                if (!controller.signal.aborted) {
    ...
    const refreshRecommendations = () => setRefresh((previous) => previous + 1)
    ...
    const priorities = [...new Set(recommendations.map((item) => item.priority).filter(Boolean))]
                ...
                <div className="flex flex-wrap items-start justify-between gap-5">
                    <div>
                        ...
                        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-purple-100">
                            Review suggested actions based on recorded inventory levels and
            ...
            {!loading && !error && data && (
                <>
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                        ...
                        <div className="rounded-2xl border border-purple-100 bg-white p-5 shadow-sm">
                            <p className="text-sm text-gray-500">Business suggestions</p>
                        ...
                        <div className="rounded-2xl border border-purple-100 bg-white p-5 shadow-sm">
                            <p className="text-sm text-gray-500">Historically busiest days</p>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-4">
                        <div>
                            <h3 className="text-xl font-bold text-purple-950">Suggested actions</h3>

                            <p className="mt-1 text-sm text-gray-500">
                                {filtered.length} of {recommendations.length} recommendations shown
                            </p>
                        </div>
