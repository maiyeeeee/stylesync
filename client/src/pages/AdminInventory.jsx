...
const input =
  "w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-800 outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100"
const primary =
...
const secondary =
  "rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
...
function AdminInventory() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      await request(
        editingProduct ? `/inventory/${editingProduct.id}` : "/inventory",
        {
  ...
  async function handleDelete(product) {
    if (
      ...
      !window.confirm(`Delete "${product.name}" from inventory?`)
    ) {
      return
    }
          ...
          <form onSubmit={handleSubmit}>
            <fieldset disabled={disabled} className="grid gap-4 md:grid-cols-2">
              ...
              <div className="flex items-center rounded-xl bg-purple-50 p-4 text-sm leading-relaxed text-purple-800">
                A restocking alert appears when stock reaches or falls below
                this product’s reorder level.
              </div>

              <div className="flex flex-wrap gap-2 border-t border-slate-100 pt-4 md:col-span-2">
                <button type="submit" className={primary}>
                  {busy ? "Saving…" : editingProduct ? "Save changes" : "Save product"}
                </button>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredProducts.length === 0 ? (
                ...
                filteredProducts.map((product) => {
                  ...
                  return (
                    <tr key={product.id} className="transition hover:bg-purple-50/40">
                      ...
                      <td className="px-4 py-4">
                        <span className={`whitespace-nowrap rounded-full px-3 py-1 text-xs font-semibold ${badge}`}>
                          {status}
                        </span>
                      </td>
                      <td className="px-6 py-4">
