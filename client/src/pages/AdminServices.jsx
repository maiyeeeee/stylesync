...
function AdminServices() {
  ...
  const categories = [
    ...new Set(
      services.map((item) => item.category || "Uncategorized")
    ),
  ].sort()
  ...
  const cards = [
    ...
    [
      "Available",
      services.filter((item) => item.status === "Available").length,
    ],
    [
      "Unavailable",
        ...
        >
          ...
          <form onSubmit={saveService}>
            <fieldset disabled={blocked} className="grid min-w-0 gap-4 md:grid-cols-2">
              ...
              <Field label={editing ? "Replace photo (optional)" : "Photo (optional)"}>
                <input
                  key={fileKey}
                  type="file"
                  accept="image/*"
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 p-5">
          <div>
            ...
            <p className="mt-1 text-xs text-gray-500">
              {loading ? "Loading…" : `${filtered.length} of ${services.length} services`}
            </p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[850px] text-left text-sm">
            <thead className="border-y border-purple-100 bg-purple-50 text-purple-900">
              <tr>
                {["Service", "Category", "Price", "Duration", "Status", "Actions"].map((label) => (
            ...
            <tbody>
              ...
              {filtered.map((item) => (
                <tr key={serviceId(item)} className="border-b border-gray-100 hover:bg-purple-50">
