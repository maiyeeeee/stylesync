...
const shortDate = (value) =>
  new Date(`${value}T00:00:00+08:00`).toLocaleDateString(
    "en-PH",
    {
      month: "short",
      day: "numeric",
...
function ModuleLink({ to, children }) {
  return (
    <Link
      to={to}
      className="text-sm font-semibold text-purple-700 hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-purple-600"
    >
      {children}
    </Link>
  )
...
function Metric({ title, value, note, to }) {
  return (
    <Link
      to={to}
      className="min-w-0 rounded-2xl border border-purple-100 bg-white p-5 shadow-sm transition hover:border-purple-300 hover:shadow-md focus-visible:outline focus-visible:outline-2 focus-visible:outline-purple-600"
    >
      <p className="text-sm font-medium text-gray-500">
        {title}
      </p>

      <p className="mt-3 break-words text-2xl font-bold tracking-tight text-purple-900">
        {value}
      </p>
...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [dashboardResult, recommendationResult, localResult] =
        results
      ...
      if (
        ...
        setEmergencyError(
          "The local emergency queue could not be read."
        )
      }
  ...
  const offShift = stats.staff.filter(
    (item) => item.availability === "Off shift"
  ).length

  const unavailable = stats.staff.filter(
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            ...
            <p className="mt-3 max-w-xl text-sm leading-relaxed text-purple-100">
              Today’s schedule, recorded sales, and the items
              that need your attention.
            </p>

            <p className="mt-4 text-xs text-purple-200">
              {shortDate(stats.today)} · Updated {updatedAt} ·
              Philippine time
            </p>
          </div>
          ...
          <button
            type="button"
            onClick={refreshDashboard}
            className="rounded-xl border border-white px-4 py-2 text-sm font-semibold hover:bg-white hover:text-purple-900"
          >
            Refresh overview
          </button>
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {[
            ...
            <Link
              key={to}
              to={to}
              className="rounded-xl bg-white px-4 py-2 text-sm font-semibold text-purple-900 transition hover:bg-purple-100"
            >
              {label}
            </Link>
          ))}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Recorded activity through today.
          </p>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <div className="min-w-0 xl:col-span-2">
          <Panel
            ...
            action={
              <ModuleLink to="/admin/appointments">
                View all →
              </ModuleLink>
            }
          >
        ...
        >
          ...
          <div className="max-h-64 space-y-3 overflow-y-auto">
            {!stats.staff.length && (
              <Empty>No active staff records.</Empty>
            )}
          ...
          <p className="mt-4 text-xs leading-relaxed text-gray-500">
            Booking still checks service qualifications and
            availability for the complete service duration.
          </p>
        </Panel>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Panel
          ...
          subtitle="Open a module to review the records"
        >
          <div className="space-y-3">
            {!attention.length && (
              <Empty>
                No alerts in the available dashboard data.
              </Empty>
            )}
        ...
        <Panel
          ...
          action={
            <ModuleLink to="/admin/recommendations">
              View all →
            </ModuleLink>
          }
        >
          {recommendationError ? (
            ...
            <Empty>
              No recommendations triggered by the current data.
            </Empty>
          ) : (
