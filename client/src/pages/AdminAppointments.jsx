...
function AdminAppointments() {
  ...
  async function mutate(url, options, successMessage) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.error || "Unable to save this change."
        )
      }
  ...
  const statuses = [
    ...new Set([
      ...
      ...appointments
        ...
        .filter(
          (status) =>
            Boolean(status) && status !== "Pending Validation"
        ),
    ]),
  ]
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-relaxed text-gray-500">
            Review assigned staff and the complete service
            interval before approval.
          </p>
        </div>
      ...
      <section className="min-w-0 overflow-hidden rounded-2xl border border-purple-100 bg-white shadow-sm">
        ...
        <p className="px-5 py-4 text-xs text-gray-500">
          ...
          Summary cards show all loaded appointments.
        </p>
      </section>
    </div>
  )
