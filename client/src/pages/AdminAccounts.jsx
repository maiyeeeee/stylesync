...
const inputClass =
  "border border-gray-300 rounded-xl px-4 py-3 w-full"
...
export default function AdminAccounts() {
  ...
  const loadAccounts = useCallback(async () => {
    ...
    try {
      ...
      const [accountData, invitationData] = await Promise.all([
        ...
        apiFetch(`${API_URL}/auth/owner-invitations`).then((res) =>
          res.json()
        ),
      ])
  ...
  async function createInvitation(event) {
    ...
    await mutate(
      ...
      (data) => {
        setInviteLink(
          `${window.location.origin}/admin-register#owner-invite=${data.token}`
        )
        setInviteEmail("")
      }
    )
  ...
  function approval(user, decision) {
    ...
    if (
      !window.confirm(
        `${action} the staff registration for ${user.username}?`
      )
    ) {
      return
    }
  ...
  function remove(user) {
    ...
    if (
      !window.confirm(
        `${warning}Permanently delete account ${user.username}?`
      )
    ) {
      return
    }
  ...
  function revoke(invitation) {
    if (
      !window.confirm(
        `Revoke the owner invitation for ${invitation.email}?`
      )
    ) {
      return
    }
    ...
    return mutate(
      `/owner-invitations/${invitation.invitation_id}`,
      { method: "DELETE" }
    )
  }
  ...
  async function copyInvitation() {
    try {
      ...
      setMessage(
        "Invitation copied. Share it privately with the real owner."
      )
    } catch {
      setError(
        "Select the invitation link and press Ctrl+C to copy it manually."
      )
    }
  }
  ...
  return (
    <>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-gray-600">
            Approve staff requests, manage accounts, and invite the
            real salon owner.
          </p>
        </div>
      ...
      {error && (
        <p
          role="alert"
          className="bg-red-100 text-red-700 rounded-xl p-4 mb-4"
        >
          {error}
        </p>
      )}
      ...
      {message && (
        <p
          role="status"
          className="bg-green-50 text-green-800 rounded-xl p-4 mb-4"
        >
          {message}
        </p>
      )}
      ...
      <section className="bg-white rounded-2xl shadow p-6 mb-6">
        ...
        <p className="text-sm text-gray-500 mb-4">
          Staff register through Register Account on the login page,
          then wait here for your approval. Verify that each person
          belongs to your salon team before approving access.
        </p>
      ...
      <section className="bg-purple-50 border border-purple-100 rounded-2xl p-6 mb-6">
        <h3 className="text-xl font-bold text-purple-800 mb-2">
          Invite the Real Owner
        </h3>

        <p className="text-sm text-purple-900 mb-4">
          The real owner will choose her own username and password.
          Invitations are private, email-specific, single-use, and
          valid for 24 hours. Keep the testing account until the new
          owner has signed in.
        </p>
        ...
        {inviteLink && (
          <div className="bg-white rounded-xl p-4 mt-4">
            ...
            <p className="text-xs text-gray-500 mt-2">
              Share privately, not in screenshots or public chats.
              The full link is not stored and disappears when you
              leave this page.
            </p>
          </div>
        )}
