...
function statusLabel(item) {
  ...
  return item.daily_status === "Available"
    ? "Marked available"
    : "Marked unavailable"
}
...
function activeSchedules(item) {
  return (item.schedules || []).filter(
    (schedule) =>
      schedule.active == null || Number(schedule.active) === 1
  )
}
...
function AdminStaff() {
  ...
  const mutate = async (
    ...
    try {
      ...
      setError(
        requestError.message || "Unable to save the change."
      )
    } finally {
  ...
  const removeLeave = (id) => {
    if (
      blocked ||
      !window.confirm("Remove this unavailable period?")
    ) {
      return
    }

    mutate(
      `/staff/unavailability/${id}`,
      { method: "DELETE" },
      "Unavailable period removed."
    )
  }

  const activeStaff = staff.filter(
    (item) => item.active_status === "Active"
  )

  const availableCount = activeStaff.filter(
    (item) => item.daily_status === "Available"
  ).length

  const unavailableCount = activeStaff.filter(
    (item) => item.daily_status !== "Available"
  ).length
  ...
  const selectableServices = services.filter(
    (service) =>
      ...
      form.service_ids.includes(Number(service.service_id))
  )
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            ...
            <p className="mt-3 max-w-xl text-sm leading-relaxed text-purple-100">
              Manage service qualifications, working shifts,
              availability settings, and time away.
            </p>
          </div>
      ...
      <p className="text-sm leading-relaxed text-gray-500">
        “Marked available” is a manual setting. It does not
        mean the person is on shift or free for a specific
        booking. Booking checks also consider qualifications,
        shifts, leave, and existing appointments. The manual
        setting remains until changed.
      </p>
      ...
      {leaveStaff && (
        ...
        >
          ...
          <p className="mt-2 text-sm text-gray-500">
            Enter salon-local dates and times. Review existing
            appointments before recording time away.
          </p>
              ...
              {item.unavailability?.length > 0 && (
                <details className="mt-4 rounded-xl border border-amber-100 bg-amber-50 p-4">
                  <summary className="cursor-pointer text-sm font-semibold text-amber-900">
                    Current / upcoming time away (
                    {item.unavailability.length})
                  </summary>
