  ...
  try {
    ...
    const identifier =
      typeof suppliedIdentifier === "string"
        ? suppliedIdentifier.trim().toLowerCase()
        : ""
  ...
  async (req, res, next) => {
    try {
      ...
      if (
        ...
        throw problem(
          "Choose a valid pending account and approval decision."
        )
      }
      ...
      if (result.affectedRows !== 1) {
        throw problem(
          "This request is no longer pending. Refresh the accounts list.",
          409
        )
      }
  ...
  async (req, res, next) => {
    try {
      ...
      if (
        ...
        throw problem(
          "Enter your current owner password to authorize the invitation."
        )
      }
  ...
  async (req, res, next) => {
    try {
      ...
      if (
        ...
        throw problem(
          "Enter the email address used for your owner invitation."
        )
      }
      ...
      res.status(201).json({
        user_id: userId,
        message:
          "Your owner account is ready. Log in using your new details.",
      })
    } catch (error) {
      registrationError(error, res, next)
    }
    ...
    try {
      ...
      const affectedRows = await transaction(async (connection) => {
        ...
        const actingOwnerExists = owners.some(
          (owner) =>
            Number(owner.user_id) === Number(req.user.user_id)
        )
        ...
        const deletingOwner = owners.some(
          (owner) => Number(owner.user_id) === userId
        )
  ...
  async (req, res, next) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error:
            "New password must contain at least 8 characters and at most 72 bytes.",
        })
      }
      ...
      if (result.affectedRows !== 1) {
        return res.status(409).json({
          error:
            "The account changed during this request. Please log in again.",
        })
      }
