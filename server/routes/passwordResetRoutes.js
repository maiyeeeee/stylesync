...
const invalidMessage = "This reset link is invalid or expired. Request a new one."
...
async function issueReset(email, mail) {
    ...
    try {
        ...
        const [users] = await connection.query(
            `SELECT user_id, email, password, role, account_status
       FROM users
       ...
       (token_hash, user_id, credential_hash, created_at, expires_at)
       VALUES (
         ?, ?, ?, UTC_TIMESTAMP(),
         DATE_ADD(UTC_TIMESTAMP(), INTERVAL 30 MINUTE)
       )`,
            [hash, user.user_id, digest(user.password)],
        )
    ...
    const link = `${mail.origin}/forgot-password#token=${delivery.token}`

    try {
        await mail.transport.sendMail({
            from: mail.from,
            to: delivery.to,
            subject: "Reset your StyleSync password",
            text:
        ...
        await database.query("DELETE FROM password_resets WHERE token_hash = ?", [delivery.hash])
        throw error
    }
}

router.post("/forgot-password", requestLimit, (req, res) => {
    ...
    // Respond before lookup so the response does not reveal accounts.
    res.json({ message: genericMessage })

    void issueReset(email, mail).catch((error) => {
...
router.post("/reset-password", resetLimit, async (req, res) => {
    ...
    if (typeof password !== "string" || password.length < 8 || Buffer.byteLength(password) > 72) {
        return res.status(400).json({
            error: "Use at least 8 characters and at most 72 bytes.",
        })
    }
    ...
    try {
        ...
        if (
            ...
            user.account_status !== "Active" ||
            !["owner", "admin"].includes(user.role) ||
            resets[0].credential_hash !== digest(user.password)
        ) {
            await connection.rollback()
            return res.status(400).json({ error: invalidMessage })
        }
        ...
        // Existing session checks invalidate the previous credentials.
        return res.json({
