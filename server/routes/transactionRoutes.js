...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (appointmentId) {
      ...
      if (existing.length) {
        throw problem("This appointment already has a transaction. Refresh the history.", 409)
      }
      ...
      if (deposits.length) {
        ...
        if (
          ...
          throw problem(
            "Resolve this booking's payment status before checkout.",
            409
          )
        }
      ...
      const expectedDeposit = cents(
        body.expected_deposit_amount,
        "Displayed deposit",
        true
      )
      ...
      const stockStatus =
        newStock <= Number(product.alertLevel) ? "Low Stock" : "In Stock"

      await connection.query(
        "UPDATE inventory SET stock = ?, status = ? WHERE id = ?",
        [newStock, stockStatus, itemId]
      )
    }
