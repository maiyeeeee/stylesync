...
if (!secret || Buffer.byteLength(secret) < 32) {
    console.error("Missing or invalid SESSION_SECRET. Generate a random secret and add it to .env.")
    process.exit(1)
}
...
function getSessionPayload(req) {
    ...
    return {
        ...
        expiresAt: req.user
            ? Math.min(Date.now() + IDLE_TIMEOUT, req.session.authenticatedAt + ABSOLUTE_TIMEOUT)
            : null,
    }
}
...
function isPublicSiteRequest(req) {
    ...
    if (
        (["GET", "HEAD"].includes(req.method) && paymentPath === "/booking-payments/reservation") ||
        (req.method === "POST" &&
            ["/booking-payments/reserve", "/booking-payments/submit"].includes(paymentPath))
    )
        return true
...
api.use(async (req, res, next) => {
    try {
        ...
        if (!Number.isFinite(authenticatedAt) || Date.now() - authenticatedAt >= ABSOLUTE_TIMEOUT) {
       ...
       LIMIT 1`,
        ...
        if (
            ...
            !["admin", "owner", "user"].includes(user.role)
        ) {
            await clearSession(req, res)
        ...
        if (req.session.credentialVersion !== credentialVersion(user.password)) {
            await clearSession(req, res)
...
api.use(
    ...
    (req, res, next) => {
        ...
        const checkingAvailability =
            ["GET", "HEAD"].includes(req.method) && currentPath === "/availability"
