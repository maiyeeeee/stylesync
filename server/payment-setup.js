...
async function main() {
  ...
  if (!columns.length) await conn.query('ALTER TABLE booking_deposits ADD COLUMN qr_snapshot LONGTEXT NULL')
  const qr = 'data:image/jpeg;base64,' + fs.readFileSync(path.join(__dirname, 'payment-assets/gcash-test.jpg')).toString('base64')
  await conn.query(`INSERT IGNORE INTO booking_payment_settings
...
main().catch(err => { console.error(err.message); process.exitCode = 1 }).finally(() => db.end())
