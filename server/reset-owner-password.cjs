...
async function main() {
    if (!process.stdin.isTTY) {
        throw new Error("Use an interactive PowerShell terminal in VS Code.")
    }

    const database = require("./db").promise()

    try {
        const [databases] = await database.query("SELECT DATABASE() AS name")

        if (databases[0]?.name !== "stylesync_db") {
            throw new Error("Wrong database selected. Expected stylesync_db.")
        }
       ...
       WHERE LOWER(TRIM(email)) = ?`,
        ...
        console.log("Typing is hidden, including stars. Press Enter after each password.")

        const password = await askHidden("New password: ")
        const confirmation = await askHidden("Confirm new password: ")

        if (password.length < 8 || Buffer.byteLength(password, "utf8") > 72) {
            throw new Error("Use at least 8 characters and at most 72 bytes. Run again.")
        }

        if (password !== confirmation) {
            throw new Error("Passwords do not match. No password changed. Run again.")
        }
       ...
       WHERE user_id = ? AND username = ?
         ...
         AND CAST(password AS BINARY) = CAST(? AS BINARY)`,
            [hash, 14, "irene", "irenecartaciano38@gmail.com", owner.password],
        )

        if (result.affectedRows !== 1) {
            throw new Error("The account changed during reset. No password changed by this script.")
        }

        console.log("Password reset successful. Log in as irene using your new password.")
    } finally {
